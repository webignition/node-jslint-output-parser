...
class ExceptionOutputTest extends BaseTest {
    ...
    public function testInputFileNotFound() {
        $this->setExpectedException(Exception::class, 'Input file "/home/example/script.js" not found', 1);
        $this->parser->parse($this->getOutput());
    }

    public function testUnexpectedOutput() {
        $this->setExpectedException(Exception::class, 'Unexpected output; is not a lint result set', 2);
        $this->parser->parse($this->getOutput());
    }

    public function testIncorrectNodeJsPathOutput() {
        $this->setExpectedException(Exception::class, 'node-jslint not found at "/home/example/node_modules/jslint/bin/jslint.js"', 3);
        $this->parser->parse($this->getOutput());
    }
